...
public abstract class BaseTest {
    ...
    public void afterEach() {
        Path videoPath = page.get().video() != null ?page.get().video().path() : null;
        context.get().close();
        browser.get().close();
        ...
        if(videoPath != null && Files.exists(videoPath)) {
            try{
                Allure.addAttachment("Test Video record", "video/webm",
                        new ByteArrayInputStream(Files.readAllBytes(videoPath)), ".webm");
            }catch (Exception e){
                System.out.println("Could not attach vide to Allure: " + e.getMessage());
            }
        }
