package io.maksym.web.api.user_tests;
...
public class UpdateUserProfileTest extends BaseTest {
    ...
    public Stream<? extends Arguments> updateUserProfileNegativeTestProvider() {
        DataGenerators generators = new DataGenerators();
        return Stream.of(
                Arguments.of("Profile with invalid Token", new UserUpdateBody(generators.generateRandomName(4, 30),  new DataGenerators().generateRandomPhone(), new DataGenerators().generateRandomCompany()), "wrongToken", UNAUTHORIZED_STATUS.getStatus(), EXPECTED_SUCCESS_FALSE, UNAUTHORIZED_MESSAGE.getMessage()),
                Arguments.of("with NULL value in [Name], [Phone], [Company]", new UserUpdateBody("", "", ""), token, BAD_REQUEST_STATUS.getStatus(), EXPECTED_SUCCESS_FALSE, NAME_MISSED_MESSAGE.getMessage())
        );
    }

    @DisplayName("Verify that user is able to update")
